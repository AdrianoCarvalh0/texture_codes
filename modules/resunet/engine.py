# this engine follows the model from: https://github.com/chcomin/Pytorch-template
...
from cldice import calculate_cldice
...
def train_one_epoch(model, data_loader_train, loss_func, optimizer, lr_scheduler, device, 
                    non_blocking, use_amp, scaler, batches_per_epoch, ds_size):
    ...
    for batch_idx, (image, target) in enumerate(data_loader_train):
        ...
        loss = loss.item()
        # Last batch might have different dimension, we multiply by batch size to calculate the 
        # average correctly
...
def validate(model, data_loader_valid, loss_func, device, ds_size):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (image, target) in enumerate(data_loader_valid):
            ...
            cldice = calculate_cldice(output, target)
            
            # Last batch might have different dimension, we multiply by batch size to calculate 
            # the average correctly
            stats = torch.tensor([loss_valid, metrics['iou'], metrics['prec'], 
                                  metrics['rec'], cldice]) * image.shape[0]
            valid_stats += stats
        valid_stats /= ds_size
...
def train(model, ds_train, ds_valid, experiment_folder, loss, class_weights, epochs, lr, 
          batch_size_train, batch_size_valid, momentum=0.9, weight_decay=0., scheduler_power=0.9, 
          device='cuda', num_workers=0, use_amp=False, pin_memory=False, non_blocking=False, 
    ...
    lr_scheduler = torch.optim.lr_scheduler.PolynomialLR(optimizer, 
                                                         total_iters=len(data_loader_train)*epochs, 
    ...
    for epoch in range(start_epoch, start_epoch+epochs):
        ...
        loss = train_one_epoch(model, data_loader_train, loss_func, optimizer, lr_scheduler, 
                               device, non_blocking, use_amp, scaler, batches_per_epoch, 
                               len(ds_train))
        valid_stats = validate(model, data_loader_valid, loss_func, device, len(ds_valid))

        time_epoch = time.time() - time_start
        print(f'\nEpoch finished in {time_epoch:.1f} seconds')
        print(f'Train loss: {loss}, Valid loss: {valid_stats[0]}, IoU: {valid_stats[1]}, '
              f'Prec: {valid_stats[2]}, Rec {valid_stats[3]}, clDice {valid_stats[4]}')
...
def run(user_params):
    ...
    if params['dataset'] == 'original':
        # Dataset
        ds_train, ds_valid = original_dataset.create_datasets(params['img_dir'], 
                                                              params['label_dir'],  
                                                              params['train_val_split'],
                                                              params['seed'])
    elif params['dataset'] == 'artificial':
        # Dataset
        ds_train, ds_valid = artificial_dataset.create_datasets(params['img_dir'],
                                                                params['label_dir'],
                                                                params['img_art_dir'],
                                                                params['label_art_dir'],
                                                                params['train_val_split'],
                                                                params['seed'])
    
    # Model
    if params['model_type']=='unet':
        model = torchtrainer.models.resunet.ResUNet(params['model_layers'], 
                                                    params['model_channels'])
    elif params['model_type']=='resnetseg':
        model = torchtrainer.models.resnet_seg.ResNetSeg(params['model_layers'], 
                                                         params['model_channels'])
    # Training
