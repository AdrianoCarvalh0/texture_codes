...
for j in range(100):
    ...
    path = f'{trein_dir}/Imagens_Artificiais/Home/pack9/imagens_artificiais/{nome_background}_{j}_com_{n_vasos}.tiff'
    img = img1.save(path)

    img2 = Image.fromarray(fundo_recortado2.astype(np.bool_))
    path = f'{trein_dir}/Imagens_Artificiais/Home/pack9/labels/{nome_background}_{j}_com_{n_vasos}.tiff'
    img = img2.save(path)
