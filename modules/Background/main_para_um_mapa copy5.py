...
for j in range(100):
    ...
    path = f'{trein_dir}/Artificial_Images/Geradas_a_partir_de_1_mapa/pack5/Artificial_Images/{nome_background}_{j}_com_{n_vasos}.tiff'
    img = img1.save(path)

    img2 = Image.fromarray(fundo_recortado2.astype(np.bool_))
    path = f'{trein_dir}/Artificial_Images/Geradas_a_partir_de_1_mapa/pack5/labels/{nome_background}_{j}_com_{n_vasos}.tiff'
    img = img2.save(path)
