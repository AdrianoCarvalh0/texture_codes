...
for i in range(5):
    ...
    img_dir = f'{root_dir}/{n}_cut.tif'

    img_label = f'{root_dir}/{n}_cut_bin.tif'

    #img = np.array(Image.open(f'{img_dir}/{images_vector[i]}'))
    #label = np.array(Image.open(f'{img_label}/{label_vector[i]}'))

    img = np.array(Image.open(f'{img_dir}'))
    label = np.array(Image.open(f'{img_label}'))
    ...
    background_name = f'{root_dir}/Images/retina/backgrounds/{n}.tiff'
